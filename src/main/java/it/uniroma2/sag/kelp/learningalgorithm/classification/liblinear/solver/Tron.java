...
public class Tron {
	...
	public Tron(final TronFunction fun_obj, double eps) {
		this(fun_obj, eps, 1000);
	}

	public Tron(final TronFunction fun_obj, double eps, int max_iter) {
	...
	public void tron(double[] w) {
		...
		while (iter <= max_iter && search != 0) {
			...
			if (actred < eta0 * prered)
				delta = Math.min(Math.max(alpha, sigma1) * snorm, sigma2
						* delta);
			else if (actred < eta1 * prered)
				delta = Math.max(sigma1 * delta,
						Math.min(alpha * snorm, sigma2 * delta));
			else if (actred < eta2 * prered)
				delta = Math.max(sigma1 * delta,
						Math.min(alpha * snorm, sigma3 * delta));
			else
				delta = Math
...
//		System.out.printf(msgFormatted, args);
	 ...
	 * @since 1.8
	 */
	private static void daxpy(double constant, double vector1[],
			double vector2[]) {
		if (constant == 0)
			return;
	...
	private static double euclideanNorm(double vector[]) {
		...
		// this algorithm is (often) more accurate than just summing up the
		// squares and taking the square-root afterwards

		double scale = 0; // scaling factor that is factored out
		double sum = 1; // basic sum of squares from which scale has been
						// factored out
		for (int i = 0; i < n; i++) {
			if (vector[i] != 0) {
